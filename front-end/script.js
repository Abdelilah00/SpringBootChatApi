...
function mssagesHistoryManager(from) {
    console.log("new Chat....");
    document.getElementById("chatBox").innerHTML = "";
    let tmp = messagesHistory.filter((msg) => msg.from === from || msg.to === from).sort((a, b) => a.time - b.time);
    tmp.forEach(msg => msg.from === from ? appendReceivedMessage(msg) : appendSenderMessage(msg));
}

function sendMessage() {
    let message = getMessageObject();
    stompClient.send("/app/chat/" + message.to, {}, JSON.stringify({
        fromLogin: message.from,
...
function connectToChat(userName) {
    ...
    let socket = new SockJS(uri + '/chat')
...
function registation() {
    ...
    $.get(uri + '/registration/' + userName, function (response) {
...
function fetchRegisteredUsers() {
    ...
    $.get(uri + '/fetchAllUsers', function (response) {
        for (let i = 0; i < response.length; i++)
            if (response[i] !== $('#userName').val())
                contact += "<li class='active'>\n" +
                    ...
                    "                                </div>\n" +
                    "                                <div class='user_info'>\n" +
                    "                                    <span class='contactUserName'>" + response[i] + "</span>\n" +
                    "                                    <p>" + response[i] + "</p>\n" +
                    "                                </div>\n" +
                    "                            </div>\n" +
                    "                       </li>"
        contact += "<script>addClickListener()</script>"
...
function appendSenderMessage(message) {
    let element = "<div class='d-flex justify-content-end mb-4'>\n" +
        ...
        "                        </div>\n" +
        "                    </div>";
    document.querySelector("body > div > div > div.col-md-8.col-xl-6.chat > div > div#chatBox").innerHTML += element;
    scrollToBottom();
}
...
function addClickListener() {
    $("#contacts li").click(function () {
        ...
        let userName = $(this).find('span:last').text();
        selectedUser = userName;
        $('#user_info>span').text(userName);
        mssagesHistoryManager(userName)
    })
}
